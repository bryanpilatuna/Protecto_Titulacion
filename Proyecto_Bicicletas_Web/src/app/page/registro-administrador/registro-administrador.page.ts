...
@Component({
  ...
  templateUrl: './registro-administrador.page.html',
  styleUrls: ['./registro-administrador.page.scss'],
})
export class RegistroAdministradorPage implements OnInit {
  ...
  async onRegister(email, password, nombre, apellido, cedula, telefono) {
    try {
      const user = await this.authSvc.registeradministrador(email.value, password.value, nombre.value, apellido.value, cedula.value, telefono.value,this.image);
      if (user) {
